...
- **`email_suppression` has its own constrained vocabulary.** `reason` admits only `unsubscribe`, `bounce`, `complaint`, `manual` (`valid_suppression_reason`); `suppression_type` only `all`, `marketing`, `transactional`. Use `SuppressionReason` / `SuppressionType`. The webhook passed Brevo's event names straight through until 2026-08-24 — `hard_bounce`, `soft_bounce`, `invalid`, `spam`, `blocked` — and five of six were illegal. This one failed loudly, not silently: the insert throws, `/webhooks/brevo` returns 500, and Brevo retries into the same failure, so only an unsubscribe could ever suppress. Brevo's three delivery-failure events now collapse to `bounce`. Dedupe on **email alone** — the table is `UNIQUE(email)`, so matching on email+reason passes the check and then violates the index.
